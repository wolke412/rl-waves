...
int main(void) {
  ...
  // Load texture
  Image grassImage = LoadImage("tiles/grass.png");
  Texture2D grassTexture = LoadTextureFromImage(grassImage);

  SetShaderValueTexture(shader, GetShaderLocation(shader, "grassTexture"),
                        grassTexture);
...
Mesh GeneratePlaneMesh(int gridSize, float quadSize) {
  ...
  float *vertices =
      MemAlloc(vertexCount * 3 * sizeof(float)); // Positions (x, y, z)
  float *texcoords =
      MemAlloc(vertexCount * 2 * sizeof(float)); // Texture coordinates (u, v)
  unsigned short *indices =
      MemAlloc(triangleCount * 3 * sizeof(unsigned short)); // Indices
